-- Force mockup page content into DB (safe re-run). Run if live site still shows legacy sections.
...
-- Ensure pages use section editor (not legacy GrapesJS HTML)
UPDATE pages SET editor_type = 'sections' WHERE slug IN ('home', 'about', 'contact') AND editor_type IS DISTINCT FROM 'sections';
